...
class FixMatch:
    ...
    def train(self, args, logger=None):
        ...
        proto_bank = {}  # save proto of each class
        ...
        for (_, x_lb, y_lb), (x_ulb_idx, x_ulb_w, x_ulb_s) in zip(self.loader_dict['train_lb'],
            ...
            with amp_cm():
                logits, ft_output = self.model(inputs)
                logits_x_lb = logits[:num_lb]
                logits_x_ulb_w, logits_x_ulb_s = logits[num_lb:].chunk(2)
                sup_loss = ce_loss(logits_x_lb, y_lb, reduction='mean')

                ft_x_lb = ft_output[:num_lb]
                ft_x_lb = ft_x_lb.detach()
                ft_x_ulb_w, ft_x_ulb_s = ft_output[num_lb:].chunk(2)
                ft_x_ulb_w = ft_x_ulb_w.detach()
                ft_x_ulb_s = ft_x_ulb_s.detach()
                del ft_output

                for class_label in range(args.num_classes):
                    idx_list = torch.arange(num_lb)[y_lb == class_label]
                    prototype = ft_x_lb[idx_list].mean(dim=0)
                    if not torch.isnan(prototype).any() and class_label in proto_bank:
                        proto_bank[class_label] = 0.999 * proto_bank[class_label] + (1 - 0.999) * prototype
                    elif not torch.isnan(prototype).any():
                        proto_bank[class_label] = prototype
                    else:
                        pass
                del prototype, idx_list

                # hyper-params for update
                T = self.t_fn(self.it)
                p_cutoff = self.p_fn(self.it)

                unsup_loss, mask, select, pseudo_lb, proto_bank = consistency_loss(args.gpu,
                                                                                   proto_bank,
                                                                                   ft_x_ulb_s,
                                                                                   logits_x_ulb_s,
                                                                                   logits_x_ulb_w,
