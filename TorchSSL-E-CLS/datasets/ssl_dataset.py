...
class ImagenetDataset(torchvision.datasets.ImageFolder):
    def __init__(self, root, transform, ulb, num_labels=-1):
        ...
        classes, class_to_idx = self._find_classes(self.root)   # def _find_classes(self, dir: str) -> Tuple[List[str], Dict[str, int]]:
        samples = self.make_dataset(self.root, class_to_idx, extensions, is_valid_file) # 元素为元组的列表，例如[('path/to/image1.jpg', 0), ('path/to/image2.jpg', 1), ('path/to/image3.jpg', 0), ...]
        if len(samples) == 0:
        ...
        if self.ulb:
            self.strong_transform = copy.deepcopy(transform)    # 添加一个深拷贝，使用深拷贝是为了确保拷贝后的内容修改不会影响原始内容
            self.strong_transform.transforms.insert(0, RandAugment(3, 5))   # 一种强增强

    def __getitem__(self, index):
        path, target = self.samples[index]  # 根据index获取samples中的元组（‘path’，‘target’）
        sample = self.loader(path)  # 使用前面定义的loader来加载图片
    ...
    def make_dataset(   # 通过将图像的地址和标签存入json文件，解决存储的数据结构爆内存的问题
            self,
            directory,
            class_to_idx,
            extensions=None,
        ...
        instances = []
        directory = os.path.expanduser(directory)   # 将路径中的波浪号（~）扩展为用户的主目录路径。如果路径中存在波浪号，它将被替换为当前用户的主目录路径。
        ...
        if self.num_labels != -1:
            with open('./sampled_label_idx.json', 'w') as f:    # 打开json文件
                json.dump(lb_idx, f)    # 将lb_idx写入到json文件中
        del lb_idx  # 释放lb_idx内容
        gc.collect()    # 清理垃圾，释放内存
        return instances
